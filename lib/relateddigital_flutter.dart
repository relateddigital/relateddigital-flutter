...
class RelatedDigital {
  final MethodChannel _channel = const MethodChannel(Constants.CHANNEL_NAME);
  ...
  Future<void> login(String userId, {Map<String, String>? properties}) async {
    await _channel.invokeMethod(Constants.M_LOGIN,
        {'exVisitorId': userId, 'properties': properties ?? {}});
  }
  ...
  void _handleUtmParameters(dynamic payload) {
    try {
      if (payload != null &&
        ...
        String? utmCampaign =
            payloadParams[Constants.VL_UTM_CAMPAIGN_PARAM];
        String? utmSource =
            payloadParams[Constants.VL_UTM_SOURCE_PARAM];
        String? utmMedium =
            payloadParams[Constants.VL_UTM_MEDIUM_PARAM];
